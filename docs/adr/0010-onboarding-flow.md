...
2. **Add the watch** — A simplified form with only `class_nbr` and `term`. Clicking **"Track this class"** from step 1 copies the example class number into the form. The form, full add page, and undo use the same browser-side `classWatchCreation` seam, which validates with `createClassWatchSchema` and calls `/api/class-watches`; that route remains authoritative and uses `create_class_watch_with_limit`, so validation and limits stay identical. Onboarding is marked complete only when the seam returns the created watch.
