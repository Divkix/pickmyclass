...
/**
 * Mint a one-time sign-in token (ticket) for a user whose credentials the app
 * has already verified server-side (password via users.verifyPassword, or a
 * freshly-registered account). The browser redeems it with clerk-react
 * `signIn.create({ strategy: 'ticket', ticket })`, which binds the session to
 * the browser's own Clerk client — keeping clerk-js state and cookies coherent.
 */
...
/**
 * Verify an email+password pair against Clerk without creating any session.
 * Returns the Clerk user on success, null on unknown email or bad password.
 * Used by the login route's loginAttemptPolicy adapter so lockout accounting
 * stays app-owned (failed_login_attempts) while credential checking moves to
 * Clerk.
 */
export async function verifyEmailPassword(
  email: string,
  password: string
): Promise<{ clerkUserId: string; externalId: string | null } | null> {
  const client = getClerkClient();
  ...
  try {
    ...
    return { clerkUserId: user.id, externalId: user.externalId ?? null };
  } catch {
    return null;
  }
}
