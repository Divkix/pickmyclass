-- Keep admin class pagination analyzable and scope aggregates to SectionRef.
-- A class number can repeat across terms, so every watcher and notification
-- count must group and join on both class_nbr and term.
...
IS 'Paginated admin class listing with SectionRef-scoped counts and static, whitelisted sorting.';
