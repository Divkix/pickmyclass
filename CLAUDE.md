...
- **PostHog project token** is a public client constant in `lib/posthog/config.ts` (same rationale as the Supabase anon key). Client init (`instrumentation-client.ts`) and server capture (`lib/posthog-server.ts`) import from there — do **not** rely on `process.env.NEXT_PUBLIC_POSTHOG_*` alone for the browser: those are build-time-only, and wrangler `vars` are runtime-only, so a deploy without the env at build time inlined `undefined` and dropped all client events. Both clients send directly to `https://us.i.posthog.com`; do not add a same-origin external rewrite because vinext forwards request credentials through external rewrites.
