...
3. **`worker.ts queue()`** consumer (config: `max_batch_size 5`, `max_concurrency 20`, `max_retries 3`, `retry_delay 60s`, DLQ `pickmyclass-dlq`). For `pickmyclass-dlq` → `handleDLQMessage` then **always ack**. Otherwise `processSection()` per message, then `classifyDisposition()` (`lib/queue/disposition.ts`) maps the try-result **or** caught error to a `Disposition` (`'ack' | 'retry'`); the consumer translates `ack→message.ack()`, `retry→message.retry()` (HTTP route translates `ack→200`, `retry→429/502` + a top-level `retryable` boolean). The single decision table:
   - success ⇒ `ack`
