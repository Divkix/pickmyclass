...
- **Authorization State** (`{ is_admin, is_disabled }`) is owned by one server-only module `lib/auth/authorization-state.ts`: `readAuthorizationState(client, userId, { cache })` (a **cached** read for the edge proxy, a **fresh** read for `verifyAdmin`/login — same `{ is_admin, is_disabled }` column set), plus `invalidateAuthorizationState(userId)` and the **30s per-isolate cache** (moved out of `proxy.ts`; it's now just a reader). The browser `AuthContext` admin hint stays separate (UI affordance only — the per-isolate cache can't cross into the browser). Boundary recorded in `docs/adr/0001-authorization-state-boundary.md`.
...
- Caching has **three layers**: `worker.ts fetch()` **edge HTML cache** (Cache API `caches.default`, anonymous GETs to `/`, `/faq`, `/about`, `/blog`, `/blog/*`, `/legal`, `/legal/*` — a HIT skips `proxy.ts` + the RSC render entirely; the big CPU saver, and the **only** thing setting `Cache-Control` on HTML), **`public/_headers`** (static assets — fonts/images `immutable` 1yr; `llms.txt`/`llms-full.txt`/og-image with SWR), and the Cloudflare edge. Its invariants (pathname+version-id key, anonymous/200/no-`Set-Cookie` only, **RSC requests excluded + only `text/html` stored** so a flight payload can't poison the entry, frozen-nonce trade-off, `EDGE_HTML_CACHE_TTL_S` 1h): `docs/adr/0009-edge-html-cache-rsc-exclusion.md` (supersedes `0003`).
...
- **`user_profiles`** is 1:1 with `auth.users` via the `on_auth_user_created` trigger and holds `is_admin`, `is_disabled`, consent timestamps, `notifications_enabled`/`unsubscribed_at`, `email_bounced`/`spam_complained`, `engagement_*`, and `onboarding_completed_at`/`onboarding_skipped_at` columns. A `prevent_user_profile_escalation` trigger silently forces privileged columns back to `OLD` for non-service roles (client writes to `is_admin` etc. **no-op silently**, not error).
- **Onboarding state** (`onboarding_completed_at`/`onboarding_skipped_at`, both nullable): new users get NULL for both and see a blocking **3-step `OnboardingModal`** on the dashboard — Step 1 "Find a class ID" (ASU catalog link), Step 2 "Add the watch" (a `SimplifiedWatchForm` with class number + term only, same 5-digit/selectable-term validation and `create_class_watch_with_limit` RPC as the dashboard's `AddClassWatch`), Step 3 "You're all set" confirmation (closes on next click → `onCompleted` adds the watch locally and drops the modal + card). Escape/backdrop/Skip (steps 1–2) all POST `/api/user/onboarding` → `skip_onboarding()` RPC (SECURITY DEFINER, authenticated; no-ops if already completed/skipped); a `completedRef` guards the step-3 close so `onCompleted` can't double-fire, and a `creating` state disables Skip/Back while the watch POST is in flight. PostHog `onboarding_started`/`onboarding_completed` fire client-side in the modal; `onboarding_skipped` fires server-side in the skip route. After skipping, a `FinishSetupCard` shows until the first watch is created (POST `/api/class-watches` sets `onboarding_completed_at` via the service client when still pending). Existing users were backfilled to `onboarding_completed_at = NOW()` at migration `20260711000000`, so they never see the modal/card. Onboarding state is exposed both via `/api/user/onboarding` GET and folded into the `/api/class-watches` GET response (`onboarding` field) so the dashboard doesn't need an extra mount-time fetch.
- **Onboarding popular-class example** (issue #300): on open, the modal GETs `/api/onboarding/popular-class`, which calls the `get_most_watched_class(p_term)` RPC (SECURITY DEFINER, service_role-only, migration `20260712000000`) for the current selectable term (`getSelectableTerms()[0]`) and validates the top SectionRef via `fetchClassFromASU`. The RPC counts only active watchers (same eligibility filters as `get_sections_to_check`) and `ORDER BY COUNT(*) DESC, class_nbr ASC LIMIT 1`. On success the modal shows a "Track this class" card (subject/catalog/title/class #) whose click sets `prefillClassNbr` and advances to step 2 (the `SimplifiedWatchForm` mounts with `defaultClassNbr`). Any failure (no selectable term, no watches, ASU 404/error, RPC error) **fails open to `popularClass: null`** and the modal shows the text-only guide instead — the fetch never blocks onboarding.
- **Watcher-eligibility filters must stay in sync** across `get_sections_to_check`, `get_watchers_for_sections`, and `get_class_watchers`: `notifications_enabled`, `NOT email_bounced`, `NOT spam_complained`, `NOT is_disabled`, `engagement_disabled_at IS NULL`. `get_class_watchers` is keyed on the full SectionRef — `(p_class_nbr, p_term)`, not `class_nbr` alone — so the admin class-detail Watchers table and the DLQ alert count don't over-list across terms (fixed in `20260701040540`). `get_watchers_for_sections` is likewise SectionRef-scoped — it takes `section_numbers` **and** `p_term`, so recipient selection for notifications can't cross terms (fixed in `20260712000001`; same defect class as `get_class_watchers`).
...
- **`proxy.ts` is the auth gate** (the sole vinext middleware file). Supabase URL/anon-key are duplicated in 3 files. Authorization State (`{is_admin,is_disabled}`) is owned by `lib/auth/authorization-state.ts` (cached edge read / fresh admin+login read); its 30s cache — invalidate via `invalidateAuthorizationState` after authz-field changes.
