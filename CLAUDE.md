...
| File | Role | Matcher |
|------|------|---------|
| **`proxy.ts`** | **THE auth gate** (vinext's middleware convention — `proxy.ts` is the only middleware file; vinext picks it over a `middleware.ts` and 0.1.7+ errors if both exist. Exported as `proxy`, `middleware`, and `default`). Thin I/O shell: **per-request CSP nonce** + all security headers + Supabase cookie-client + `getUser()` + **cached** `readAuthorizationState` (30s per-isolate cache owned by `lib/auth/authorization-state.ts`). Pure routing/redirect logic lives in `lib/auth/decide-gate.ts` (`decideGate` → `GateDecision`); `proxy.ts` only translates the one decision to `NextResponse` and applies headers once. | catch-all |
...
- Routing/redirect decisions live in `lib/auth/decide-gate.ts` (`decideGate` pure, testable without Supabase/NextResponse); `proxy.ts` is the sole edge gate and only handles nonce/CSP/cookie I/O + `addSecurityHeaders` once.
- Security/CSP/nonce still live in `proxy.ts` (single header application via one exit, not 9).
...
**Key `lib/` sub-modules:** `asu/` (ASU client + terms), `api/` (zod schemas + `ok()/fail()` envelope + validation), `auth/` (authorization-state + `decide-gate` pure routing), `class-watches/` (browser creation seam), `db/` (Supabase queries + admin queries), `supabase/` (3 clients + generated types), `queue/` (process-section with `SectionCheckOutcome` disposition, change-detector, notification-sender, dlq-consumer), `worker/` (edge runtime modules), `email/` (send + templates + auth-templates + unsubscribe-token), `cache/`, `hooks/`, `contexts/`, `types/`, `blog/`, `config.ts`, `log.ts`, `animations.ts`.
