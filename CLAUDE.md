...
**Key `lib/` sub-modules:** `asu/` (ASU client + terms), `api/` (zod schemas + `ok()/fail()` envelope + validation), `auth/`, `class-watches/` (browser creation seam), `db/` (Supabase queries + admin queries), `supabase/` (3 clients + generated types), `queue/` (process-section, change-detector, notification-sender, dlq-consumer), `email/` (send + templates + auth-templates + unsubscribe-token), `cache/`, `hooks/`, `contexts/`, `types/`, `blog/`, `config.ts`, `log.ts`, `animations.ts`.
