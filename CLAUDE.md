...
**PickMyClass** notifies ASU students by email when a seat opens (or an instructor is assigned) in a class section they watch. It's a **Next.js 16 App Router** app (React 19, TypeScript strict) deployed on **Cloudflare Workers via the `vinext` adapter** (Vite-based Next.js — *not* `@cloudflare/next-on-pages` or OpenNext), backed by **PlanetScale Postgres via Cloudflare Hyperdrive (`pg` 8.23, `--caching-disabled`, 5-conn pool) + Clerk** (networkless `jwtKey` verification, `ext_id` claim `{"ext_id":"{{user.external_id || user.id}}"}`) + **polling** (Realtime removed, `docs/adr/0014-realtime-to-polling.md`), with **Cloudflare Email Service** for all transactional email and **Cloudflare Queues + a Durable Object** for the seat-checking pipeline. Tooling is **Vite+ (`vp`)** and the package manager is **`pnpm@11.10.0`**.

The data plane is Hyperdrive `pg` (see `lib/db/client.ts`; 5-conn pool, `--caching-disabled`; `docs/adr/0013-data-access-hyperdrive.md`); the auth plane is Clerk (see `lib/auth/clerk-session.ts` + `lib/db/users.ts` mirror + `lib/clerk/config.ts` literal `CLERK_PUBLISHABLE_KEY`; Svix webhook at `/api/webhooks/clerk`; `docs/adr/0012-auth-plane-clerk.md`). Secrets are `CLERK_*` (see `lib/cloudflare-env.supplemental.d.ts` + `lib/supabase/config.ts` deprecation). `lib/supabase/client.ts`/`server.ts` are deprecated and knip-ignored.
...
```
Browser ──> vinext app (Cloudflare Worker: worker.ts) ──> PlanetScale Postgres via Hyperdrive (pg, polling)
                          │         │                               ▲
...
Files: `app/api/auth/*` (`register`, `login`, `signout`, `check-lockout`, `consent`, `email-verified`), `app/api/webhooks/clerk/route.ts` (Svix), `app/auth/post-oauth/route.ts` + `app/auth/callback/page.tsx` (`AuthenticateWithRedirectCallback`), `lib/auth/clerk-session.ts` + `lib/auth/clerk-cookies.ts` + `lib/auth/authorization-state.ts` + `lib/auth/require-user.ts`, `lib/clerk/config.ts` + `components/ClerkClientProvider.tsx`, `lib/db/users.ts` mirror, `lib/contexts/AuthContext.tsx` compat, `proxy.ts` + `lib/auth/decide-gate.ts`, `user_profiles`/`failed_login_attempts` migrations. **No `logout`/`reset-password`/`send-email-hook` server routes.**

- **Register** (`/api/auth/register`): zod `registerSchema` (password ≥8, `ageVerified===true`, `agreedToTerms===true`), lowercase email, disposable-email KV check (**fails open**), then `getClerkClient().users.createUser({ emailAddress, password, externalId? })` + `public_metadata { age_verified, agreed_to_terms }`. Duplicate email is `ClerkAPIResponseError` (`@clerk/backend/errors`, code `form_identifier_exists` 422) ⇒ `ok(null)` anti-enumeration (same as 409 path was before, now via `@clerk/backend`). Consent is in `public_metadata`; webhook or `upsertUserFromClerkWebhook` persists it (no `handle_new_user()` trigger, no `auth.uid()` RLS). See `docs/adr/0012-auth-plane-clerk.md`.
- **Login** (`/api/auth/login`): `loginAttemptPolicy.attempt(email, authenticate)` seam only maps HTTP; policy owns normalization, pre-check (locked ⇒ `423`), expiry, atomic `increment_failed_attempts`, remaining math, clear-after-success. `authenticate` calls `verifyEmailPassword(email,password)` + **fresh** `readAuthorizationState(userId, {cache:false})` (`is_disabled` ⇒ `revokeSession` + `403`). On success `createSignInTicket(userId)` → client `signIn.create({strategy:'ticket'})` sets Clerk session. `failed_login_attempts` stays RLS `service_role` only, 5/15m.
- **`check-lockout`**: unauthenticated, **omits `attempts` count** (SEC-02), no in-app rate limit — **Cloudflare WAF ~20 req/min/IP** is the guard (don't remove, don't add `attempts`). `docs/adr/0007-check-lockout-enumeration-guard.md`.
- **Sessions:** `lib/auth/clerk-session.ts` owns `getClerkClient` (per-isolate cache + `jwtKey` PEM networkless `authenticateRequest`/`verifyToken` under `workerd`; spike was §0) + `getSessionIdentity(request)` / `getSessionIdentityFromHeaders(headers)` reading `ext_id` claim (`{{user.external_id || user.id}}`, fallback `sub`) + `revokeSession`/`revokeAllUserSessions`/`createSignInTicket`/`verifyEmailPassword`. Fast-path `hasClerkSessionCookies(header)` (`lib/auth/clerk-cookies.ts`, prefixes `__session/__client_uat/__refresh/__clerk_db_jwt/__clerk_handshake`, clear list `CLERK_COOKIES_TO_CLEAR`). `requireUser()` throws `UnauthorizedError(401)` via Clerk headers. Client `AuthContext` is a compat layer: `useUser`/`useClerkAuth` → `CompatUser`/`CompatSession` (`email_confirmed_at` from `verification.status`), `isAdmin` UI affordance only, `signOut` = `clerk.signOut()` → `fetch /api/auth/signout` (AuthButton does `clerk.signOut()` **before** server revoke, then `window.location.href='/login'`).
- **Email verification / OAuth:** `POST /api/auth/email-verified` marks `markEmailVerified`; `GET /auth/post-oauth` (allow-listed in `decide-gate`) completes OAuth consent + `ensureUserMirror`; `app/auth/callback/page.tsx` is Clerk's `AuthenticateWithRedirectCallback`. OAuth Google uses `oauth_google` strategy, re-links by verified email.
- **Webhooks:** `POST /api/webhooks/clerk` Svix `verifyWebhook` (`CLERK_WEBHOOK_SIGNING_SECRET` `whsec_...`), `user.created/updated/deleted` → `upsertUserFromClerkWebhook` (`appUserId = external_id ?? id`, email lowercasing, `clerk_user_id` unique, `softDeleteUserById`). Keep `prevent_user_profile_escalation` (non-service writes no-op).
- **Consent / admin / delete:** `/api/auth/consent` re-validates both confirmations, records missing timestamps, `invalidateAuthorizationState`; `proxy.ts` blocks `has_consent=false` from protected pages. `verifyAdmin()` (RSC `redirect()`) uses fresh `readAuthorizationState`. `DELETE /api/user/delete` soft-deletes via `softDeleteUserById` + `invalidateAuthorizationState` + `revokeAllUserSessions`; `GET /api/user/export` returns JSON `Cache-Control: no-store`. See `docs/adr/0001-authorization-state-boundary.md`.
- **Password reset:** no server route — Clerk `reset_password_email_code` → `/reset-password` (allow-listed while unconfirmed, excluded from `AUTH_PAGES`, `docs/adr/0008-password-reset-no-server-route.md`).
...
- Routing/redirect decisions live in `lib/auth/decide-gate.ts` (`decideGate` pure, testable without Clerk/NextResponse); `proxy.ts` is the sole edge gate and only handles nonce/CSP/cookie I/O + `addSecurityHeaders` once.
...
## Data layer — PlanetScale Postgres via Hyperdrive (`pg`)

**Single seam** `lib/db/client.ts` (Hyperdrive `pg` Pool 5, `--caching-disabled`, `setConnectionStringGetter` registered inside `worker.ts` handlers). All DB access goes via this seam or the RPC helpers in `lib/db/queries.ts` / `lib/db/admin-queries.ts` / `lib/db/users.ts` — **no PostgREST, no `@supabase/supabase-js`**. Details `docs/adr/0013-data-access-hyperdrive.md`.
...
- **Auth helpers** (`lib/auth/`): `requireUser` (Clerk `getSessionIdentity`, throws 401) + `verifyAdmin` (RSC `redirect()`). `verifyCronSecret` uses `timingSafeCompare` (SHA-256 then `timingSafeEqual`). `loginAttemptPolicy` is the lockout seam (in-memory adapter in tests, pg store in prod via `lib/db/client.ts`). Disposable-email **fails open** at every layer and caches the KV blocklist for 1h (`_resetCache()` in tests). Clerk seams: `lib/auth/clerk-session.ts` (per-isolate `getClerkClient` + `jwtKey`) + `lib/auth/clerk-cookies.ts` (prefix detection + `CLERK_COOKIES_TO_CLEAR`) + `lib/clerk/config.ts` (literal `CLERK_PUBLISHABLE_KEY` + CSP). See `docs/adr/0012-auth-plane-clerk.md`.
...
- **Bindings** (`wrangler.jsonc` + `lib/types/env.ts`): `HYPERDRIVE` (`4dd6f09…` PlanetScale Postgres, `--caching-disabled`), `PICKMYCLASS_QUEUE` (→ `pickmyclass-queue`), DLQ `pickmyclass-dlq`, `PICKMYCLASS_CRON_LOCK_DO` (DO), `PICKMYCLASS_DISPOSABLE_DOMAINS` (KV blocklist, daily sync from GitHub; requires ≥1000 domains before overwrite), `EMAIL` (Cloudflare Email Service, remote), `ASSETS`, `CF_VERSION_METADATA` (`version_metadata` — deploy version id for edge HTML cache key). Vars: `MAX_WATCHES_PER_USER` (10), `NOTIFICATION_FROM_EMAIL`, `NEXT_PUBLIC_SITE_URL`.
...
|------|---------|-------------------|
| `lib/supabase/database.types.ts` | `pnpm exec supabase gen types typescript --project-id osopxwuebsefhoxgeojh > lib/supabase/database.types.ts` | After any `supabase/migrations/*` change (SQL history; RPC names are still Hyperdrive-queried via `pg`). Remove `as unknown as` casts once types exist. |
...
**Secrets** never appear in generated CF types — hand-type them in `lib/cloudflare-env.supplemental.d.ts` (augments `Cloudflare.Env` + `NodeJS.ProcessEnv`). `lib/supabase/config.ts` is deprecated (Supabase project no longer exists).
...
- **`proxy.ts` is the auth gate** (sole vinext middleware; Clerk `authenticateRequest` with `jwtKey`, `hasClerkSessionCookies` fast-path, `ext_id` claim). Authorization State (`{is_admin,is_disabled,has_consent}`) owned by `lib/auth/authorization-state.ts` (cached edge read / fresh admin+login read, 30s per-isolate cache); invalidate via `invalidateAuthorizationState` after consent/admin changes. `lib/clerk/config.ts` literal `CLERK_PUBLISHABLE_KEY` is the only publishable key.
...
`README.md` and `CONTEXT.md` were drift-pruned for Hyperdrive + Clerk (2026-08-22): architecture now shows PlanetScale via `HYPERDRIVE` + `CLERK_*` + polling (not Supabase Realtime), `# tech stack` lists `pg`/`@clerk/backend`/`@clerk/clerk-react` (not `@supabase/supabase-js`), and generated-files note `lib/supabase/` deprecation. Remaining drift risk is hard numbers/prose — confirm against `wrangler.jsonc`/code. History: queue "internal HTTP" → direct `processSection()` (ADR 0006), dedup "INSERT…ON CONFLICT" → partial-index + `try_record_notifications_batch` (ADR 0004), Realtime → polling (ADR 0014), Supabase Auth → Clerk (ADR 0012), PostgREST → Hyperdrive `pg` (ADR 0013).
